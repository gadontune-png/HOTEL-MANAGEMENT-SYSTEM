      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-1 mt-3">
          {room.amenities.slice(0, 3).map((a, i) => (
            <span key={i} className="bg-amber-50 text-amber-700 text-xs px-2 py-0.5 rounded-full">
              {a}
            </span>
          ))}
        ...
        <div className="flex items-center justify-between mt-5">
          <p className="text-amber-700 font-bold text-lg">
            ${room.price}<span className="text-gray-400 font-normal text-sm">/night</span>
          </p>
          {room.status === 'available' ? (
            <Link
              to="/booking"
              className="bg-amber-900 hover:bg-amber-700 text-white text-sm px-5 py-2 rounded-full transition-colors"
            >
              Book Now
            </Link>
          ) : (
            <span className="text-gray-400 text-sm px-5 py-2 border border-gray-200 rounded-full">
              Unavailable
            </span>
          )}
        </div>
